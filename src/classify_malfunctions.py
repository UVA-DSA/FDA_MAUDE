"""Classify da Vinci MDR malfunctions and their impacts from event narratives.

Reads the merged MAUDE CSV produced by download_maude_data.py and walks the
Event Description and Narrative of every report, extracting malfunction classes
(system errors, falls, arcing, broken instruments, vision problems, unintended
motion, ...) and their impacts (troubleshooting, system reset, conversion,
rescheduling, injuries). Writes a classified CSV/XLS consumed by the R analysis.

Ported from Python 2 to Python 3. Requires: nltk (with the 'punkt' tokenizer),
...
# NegEx trigger rules. When present, NegEx is used as a supplementary negation
# check on top of the hand-written negation_check() heuristic (see below). Set
# USE_NEGEX = False to fall back to the heuristic alone.
USE_NEGEX = True
NEGEX_TRIGGERS = os.path.join(DICT_DIR, 'negex_triggers.txt')
irules = []
if os.path.exists(NEGEX_TRIGGERS):
    with open(NEGEX_TRIGGERS) as rfile:
        irules = sortRules(rfile.readlines())
    print('Loaded %d NegEx trigger rules.' % len(irules))
else:
    print('Warning: negex_triggers.txt not found at %s; NegEx rules disabled.' % NEGEX_TRIGGERS)
...
Typoed_MDRs = ['349101', '626290', '1609392', '2113223', '1227009', '1063554', '1063553',
               ...
               '1671251', '2468265', '2468249', '2426441', '2940423', '2823390', '3306069',
               '2942665', '2673688', '1595504', '919538', '919534', '1804859', '930610',
               '930611', '869814', '886699', '2260987', '1707866']
Other_Missed = ['2504745', '1735914', '933211', '870229', '2120175', '1466302', '823634',
                '704959', '690620', '660137', '310796']
...
Body_Keywords = ['patient', ' pt', 'artery', 'tissue', ' vein', ' skin', 'vessel', 'cavity',
                 ...
                 'thymus gland', 'rectum', 'colon', 'nurve']

surgery_stop_adjs = ['surgical', 'surdical', 'surgery', 'operative', 's', 'si', 'da', 'vinci',
                     'davinci', 'robotic', 'during', 'the', 'of', 'on', 'for', 'and', 'demo',
                     'a', 'an', 'intended', 'training', 'open', 'clinical', 'post', 'before',
                     'after', 'pre', 'post', 'previous', 'endoscopic', 'complete', 'laparoscopic',
                     'laparoscopy', 'original', '2000', '1', 'hour', 'assisted']
...
error_keywords = ['system error', 'error code', 'error message', 'critical error',
                  'intermittent error', 'system fault', 'fault code', 'fault message',
                  'critical fault', 'intermittent fault', 'recoverable fault',
                  'non-recoverable fault', 'nonrecoverable fault', 'unrecoverable fault',
                  'recoverable error', 'non-recoverable error', 'nonrecoverable error',
                  'unrecoverable error', 'recoverable safety state', 'non-recoverable safety state',
                  'safety state', '"instrument not recognized" error message', 'fault error mode',
                  'fault alarm' 'fault appeared', 'fault displayed', 'error appeared',
                  'error displayed']
error_keywords2 = ['error code', 'error message', 'system_fault', 'fault code', 'fault message',
                   ...
                   'recoverable fault', 'non-recoverable fault', 'nonrecoverable fault',
                   'unrecoverable fault', 'recoverable error', 'non-recoverable error',
                   'nonrecoverable error', 'unrecoverable error', 'recoverable safety state',
                   'non-recoverable safety state', 'safety state', 'fault appeared',
                   'fault displayed', 'error appeared', 'error displayed']

vision_keywords = ['no vision', 'no video', 'no image', 'low light', '"dim" light', 'lack of light',
                   'little light', 'vision problem', 'vision issue', 'double vision', 'video issue',
                   'video problem', 'illuminator', 'vision system error', 'vision loss',
                   'loss of vision', 'loss of video', 'video loss', 'image loss', 'lost vision',
                   'lost the vision', 'lost video', 'lost the video', 'lost image',
                   'lost the image' 'lost 3D', 'loss of 3D', 'loss of image',
                   'light source was very dim', 'light source was dim', 'vision was lost',
                   'video was lost', 'image was lost', 'vision lost', 'video lost', 'image lost',
                   'blurry', 'foggy', 'fogging', 'tint', 'right eye', 'left eye', 'red eye',
                   'black and white balance', 'image was not aligned', 'black image', 'foggy image',
                   'red image', 'left eyes', 'right eyes', 'image', 'flicker', 'pink', '3d camera',
                   '3d image', '3d vision', 'unable to calibrate the camera',
                   'unable to calibrate the endoscope', 'issue with the camera calibration',
                   'camera failed', 'camera error', 'lens was missing', 'video signal',
                   'video process assembly', 'touchscreen', 'touch screen', 'assistant monitor',
                   'vision side cart shutdown', 'unable to focus', 'focus controller',
                   'not able to focus', 'difficulty focusing', 'difficulties focusing',
                   'focusing issue', 'could not focus', 'could not be focused',
...
fallen_keywords1 = ['fell inside', 'fell into', 'falling into', 'falling inside', 'fallen into',
                    'fallen inside', 'fall on the pt', 'fall on the patient', 'fall on the user',
                    'fall on a pt', 'fall on a patient', 'fall on a user',
                    'into the pt', 'into the patient', 'into a pt', 'into a patient',
                    "in pt's abdomen", 'fell in the pt', 'fell in the patient', 'found in the pt',
                    'found in the patient', 'inside the patient', 'inside the pt', 'inside a pt',
                    'inside a patient', 'inside of the patient', 'inside of the pt',
                    'inside of a pt', 'inside of a patient']
...
moving_keywords = {('not', 'NG'), ("didn't", 'NG'), ('stopped', 'NG'),
                   ('move', 'V'), ('moved', 'V'), ('moving', 'V'), ('functioning', 'V'),
                   ('go off', 'V'), ('open', 'V'), ('rotate', 'V'), ('close', 'V'), ('work', 'V'),
                   ('grasp', 'V'), ('stop', 'V'), ('respond', 'V'), ('opening', 'V'),
                   ('rotating', 'V'), ('closing', 'V'), ('working', 'V'), ('grasping', 'V'),
                   ('stopping', 'V'), ('by', 'IN'), ('wrong', 'IN'), ('no', 'IN'),
                   ('without', 'IN'), ('non-intuitive', 'IN'), ('unintended', 'IN'),
                   ('sudden', 'IN'), ('unexpected', 'IN'), ('uncontrolled', 'IN'), ('erratic', 'IN'),
...
Instruments = [' piece', ' fragment', ' cable', ' wire', ' tip', 'tip cover', 'tip cover accessory',
               'needle', 'pin', ' clamp ', ' insulation', 'forcep', 'forceps', 'cautery', 'screw',
               'scissor', 'distal', ' blade ', ' sleeve ', 'curved shears', 'cannula', 'grasper',
               'spatula', 'grip', ' distal ', 'trocar', 'electrocautery', 'tube', 'endoscope',
               'camera', 'battery', 'insturment', 'accessory', 'instrument', 'component', 'device']
                    ...
                    for EC in s_digits:
                        ...
                        if Curr_Error_Codes.count(EC) == 0:
                            ...
                            if EC in Error_Code_List:
                                Error_Code_List[EC] = (Error_Code_List[EC][0] + 1,
...
#### Vision Problems
    ...
    if ((Vision.find('no video') != -1) and
            ((CEvent.find('no video tape') != -1) or (CEvent.find('no video recording') != -1) or
        ...
        for s in SEvent + SNarrative:
            if (s.find('broke off') > -1) or (s.find('break off') > -1) or (s.find('broken off') > -1):
                for inst_k in Instruments:
                  ...
                  (s.find('pieces were retrieved') > -1) or (s.find('pieces were recovered') > -1) or
                  (s.find('fragments were removed') > -1) or
                  (s.find('fragments were retrieved') > -1) or
                  (s.find('fragments were recovered') > -1) or (s.find(' shredd') > -1) or
                  (s.find('fragment was removed') > -1) or (s.find('fragment was retrieved') > -1) or
                  (s.find('piece was removed') > -1) or (s.find('piece was retrieved') > -1) or
                  (s.find('fragment removed') > -1) or (s.find('fragment retrieved') > -1) or
                  (s.find('piece removed') > -1) or (s.find('piece retrieved') > -1) or
                  (s.find('fragments removed') > -1) or (s.find('fragments retrieved') > -1) or
                  (s.find('pieces removed') > -1) or (s.find('pieces retrieved') > -1) or
    ...
    for s in SEvent:
        ...
        if Moved == 'N/A':
            if (s.find('instrument') != -1) and ((s.find('not respond') != -1) or
                ...
                Moved = 'not respond'
            elif ((s.find('no recognize') != -1) or (s.find('not cauterize') > -1) or
                  (s.find('not line up') > -1) or (s.find('not match up') > -1) or
                  (s.find('not coagulate') > -1) or (s.find('not retract') > -1) or
                  (s.find('not cut') > -1) or (s.find('not engage') > -1) or
                  (s.find('not align') > -1) or (s.find('not seal') > -1)):
                Moved = 'not !!!'
